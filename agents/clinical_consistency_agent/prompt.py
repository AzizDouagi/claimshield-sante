"""Prompt système versionné du Clinical Consistency Agent."""
...
class ClinicalConsistencyPrompt:
...
@lru_cache(maxsize=1)
def load_clinical_consistency_prompt() -> ClinicalConsistencyPrompt:
    ...
    if version != PROMPT_VERSION:
        raise ValueError(
            f"Version de prompt clinical_consistency_agent inattendue : {version!r} "
            f"(attendu {PROMPT_VERSION!r})"
        )
    return ClinicalConsistencyPrompt(
        version=version,
